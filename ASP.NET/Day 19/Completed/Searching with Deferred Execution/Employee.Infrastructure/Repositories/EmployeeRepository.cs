...
namespace Employee.Infrastructure.Repositories
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        {
            if((DepartmentId <= 0 && String.IsNullOrWhiteSpace(Query)) )
                return _dataContext.Employee.ToList();

            var collection = _dataContext.Employee as IQueryable<EmployeeEntity>;

            if (DepartmentId > 0)
                collection = collection.Where(x => x.DepartmentId.Equals(DepartmentId));
            if (!string.IsNullOrWhiteSpace(Query))
                collection = collection.Where(x => ( (!String.IsNullOrWhiteSpace(x.Name) && x.Name.Contains(Query))
                || (!String.IsNullOrWhiteSpace(x.PrimaryEmailAddress) && x.PrimaryEmailAddress.Contains(Query))));
